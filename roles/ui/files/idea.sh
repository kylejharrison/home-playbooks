...
if [ "$IS_EAP" = "true" ]; then
  ...
  if [ -r "$IDE_BIN_HOME/lib$AGENT_LIB.so" ]; then
    AGENT="-agentlib:$AGENT_LIB=disablej2ee,disablealloc,delay=10000,sessionname=IntelliJIdea14"
  fi
fi

COMMON_JVM_ARGS="-XX:ErrorFile=$HOME/java_error_in_IDEA_%p.log \"-Xbootclasspath/a:$IDE_HOME/lib/boot.jar\" -Didea.paths.selector=IntelliJIdea14 $IDE_PROPERTIES_PROPERTY"
IDE_JVM_ARGS=""
